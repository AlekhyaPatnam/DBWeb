<template>
  <div class="container">
      <h4 class="listheading">InActive Student List</h4>
    <v-data-table :headers="headers" :items="studentsList" class="elevation-1">
      <template v-slot:items="props">
        ...
        <td><v-btn class="loginbutton" color="success" @click="makeInactive(props.item.uid)">Active</v-btn></td>
      </template>
    </v-data-table>
  </div>
</template>
...
export default {
    ...
    created() {
        axios
        .get("http://localhost:3001/api/getinactive")
        .then((response) => {
    ...
    methods: {
        ...
        makeInactive(uid) {
            axios
            .post("http://localhost:3001/api/makeactive", {sid:uid})
